...
NAV_FUNCTION_DESC = {
    ...
    "function": {
        "name": "navigate_to",
        "description": "开始导航到目的地。优先使用客户端提供的当前位置作为起点；如未提供，则等待后续位置更新再规划路线。",
        "parameters": {
            "type": "object",
            "properties": {
                "destination": {"type": "string", "description": "目的地名称或经纬度(如: 116.481499,39.990475)"},
...
UPDATE_FUNCTION_DESC = {
    ...
    "function": {
        ...
        "description": "导航位置更新。客户端应定期上传当前经纬度用于触发语音提示与导航进度。",
        "parameters": {
            "type": "object",
            "properties": {
...
def _set_navigation_state(conn, state):
    """设置当前连接的导航状态"""
    session_id = getattr(conn, 'session_id', 'default')
    NAVIGATION_SESSIONS[session_id] = state


def _clear_navigation_state(conn):
    """清除当前连接的导航状态"""
    session_id = getattr(conn, 'session_id', 'default')
    NAVIGATION_SESSIONS.pop(session_id, None)


@register_function("navigate_to", NAV_FUNCTION_DESC, ToolType.SYSTEM_CTL)
def navigate_to(conn, destination: str, mode: str = "driving", origin: str = None):
    """开始导航到目的地"""
    try:
        ...
        logger.bind(tag=TAG).error(f"导航开始失败: {e}")
        return ActionResponse(Action.RESPONSE, None, "导航服务暂时不可用，请稍后重试")


@register_function("navigation_update", UPDATE_FUNCTION_DESC, ToolType.SYSTEM_CTL)
def navigation_update(conn, lat: float, lon: float):
    """更新导航位置"""
    try:
        nav_state = _get_navigation_state(conn)
        if not nav_state:
            return ActionResponse(Action.RESPONSE, None, "当前没有活跃的导航")
